...
const BqTabGroup = class {
    ...
    checkPropValues() {
        props.validatePropValue(bqTab_types.TAB_SIZE, 'medium', this.el, 'size');
        props.validatePropValue(bqTab_types.TAB_ORIENTATION, 'horizontal', this.el, 'orientation');
        props.validatePropValue(bqTab_types.TAB_PLACEMENT, 'start', this.el, 'placement');
        this.bqTabElements.forEach((bqTabElement) => {
            ...
            bqTabElement.active = !isNil.isNil(this.value) ? bqTabElement.tabId === this.value : false;
        });
    }
    // Events section
    ...
    onKeyUp(event) {
        const { target } = event;
        if (!isHTMLElement.isHTMLElement(target, 'bq-tab'))
            return;
    ...
    focusTabSibling = async (currentTarget, direction) => {
        ...
        this.bqTabElements.forEach((bqTabElement, index, elements) => {
            ...
            if (bqTabElement === currentTarget) {
                target = getNextElement.getNextElement(elements, index, direction);
            }
        });
        if (target) {
            await target.vFocus();
...
BqTabGroup.style = BqTabGroupStyle0;
