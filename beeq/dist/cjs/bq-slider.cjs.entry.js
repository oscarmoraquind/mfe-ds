...
const BqSlider = class {
    ...
    handleGapChange(newValue) {
        ...
        const value = !index$1.isNil(this.min) && !index$1.isNil(this.max) ? [this.min, this.max] : this.stringToObject(this.value);
    ...
    setState = (newValue) => {
        ...
        this.minValue = isRangeType ? index$1.clamp(value[0], this.min, this.max - this.gap) : value;
        this.maxValue = isRangeType ? index$1.clamp(value[1], this.minValue + this.gap, this.max) : this.minValue;
    };
    setThumbPosition = () => {
    ...
    stringToObject = (value) => (index$1.isString(value) ? JSON.parse(value) : value);
    ...
    emitBqChange = () => {
        ...
        this.debounceBqChange = index$1.debounce(() => this.bqChange.emit({ value, el: this.el }), this.debounceTime);
    ...
    renderInput = (type, value, refCallback) => {
        ...
        return (index.h("input", { type: "range", class: {
    ...
    renderTooltip = (value, thumbPosition, refCallback) => (index.h("bq-tooltip", { class: {
            'absolute [&::part(panel)]:absolute': true,
            hidden: !this.isTooltipAlwaysVisible,
        }, exportparts: "base,trigger,panel", alwaysVisible: true, distance: this.enableValueIndicator ? 6 : 16, style: { insetInlineStart: `${thumbPosition}px`, fontVariant: 'tabular-nums' }, ref: refCallback }, index.h("div", { class: "absolute bs-1 is-1", slot: "trigger" }), value.toFixed(this.decimalCount)));
    // render() function
    // Always the last one in the class.
    // ===================================
    render() {
        return (index.h("div", { key: '612b392ec0c3e4f2e6f2ea05d08206a55f71d2c9', "aria-disabled": this.disabled ? 'true' : 'false', class: { 'flex is-full': true, 'cursor-not-allowed opacity-60': this.disabled }, part: "base" }, this.renderLabel(this.minValue, 'start', 'me-xs text-end'), index.h("div", { key: '32ce9531675176c799f1f4b7624f78abcc9b7a68', class: "relative is-full", part: "container" }, index.h("span", { key: 'a8b2f0acfba7d5d43f699f7963ce7938de3a9744', class: "absolute start-0 -translate-y-1/2 rounded-xs bg-[--bq-slider--trackarea-color] bs-1 is-full inset-bs-[50%]", ref: (elem) => (this.trackElem = elem), part: "track-area" }), index.h("span", { key: '95913f9cce99d2b807c68878a8c0d83ec59dbf76', class: "absolute -translate-y-1/2 rounded-xs bg-[--bq-slider--progress-color] bs-1 is-[50%] inset-bs-[50%]", ref: (elem) => (this.progressElem = elem), part: "progress-area" }), this.enableTooltip &&
...
BqSlider.style = bqSliderCss;

exports.bq_slider = BqSlider;

//# sourceMappingURL=bq-slider.cjs.entry.js.map