...
const BqSelect = class {
    ...
    // =======================
    handleValueChange() {
        if (this.multiple && index$1.isString(this.value)) {
    ...
    stopOptionFocusBlurPropagation(ev) {
        ...
        if (index$1.isHTMLElement(ev.target, 'bq-select'))
            return;
        ev.stopPropagation();
    }
    // Public methods API
    ...
    handleSearchFilter = (ev) => {
        ...
        if (!index$1.isDefined(query)) {
            this.clear();
        }
        else {
            this.debounceQuery = index$1.debounce(() => {
    ...
    get hasClearIcon() {
        ...
        return index$1.isDefined(this.displayValue);
    }
    // render() function
