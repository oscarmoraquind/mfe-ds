...
const index$1 = require('./index-1e4795f3.js');
const index = require('./index-8c48e726.js');
...
const fetchSvg = async (url, sanitize) => {
    if (index.isNil(url) || typeof fetch === 'undefined' || typeof window === 'undefined') {
        return;
    }
    if (requests.has(url))
...
const validateContent = (svgContent) => {
    if (typeof window === 'undefined' || index.isNil(svgContent))
        return '';
    const svgTag = 'svg';
...
const isValid = (elm) => {
    ...
    for (const attribute of Array.from(elm.attributes)) {
        if (index.isString(attribute.value) && attribute.value.toLowerCase().startsWith('on')) {
            return false;
        }
    }
...
const BqIcon = class {
    constructor(hostRef) {
        ...
        this.svgLoaded = index$1.createEvent(this, "svgLoaded", 7);
    }
    // Own Properties
    // ====================
    ...
    handleWeightChange() {
       ...
       For example, '<bq-icon name="bell-fill"></bq-icon>' instead of '<bq-icon name="bell" weight="fill"></bq-icon>'`);
        ...
        const isWeightedIcon = !index.isNil(this.weight) && this.weight !== REGULAR;
    ...
    getIconSource = (name) => {
        ...
        return index.getBasePath(iconFileName);
    };
    loadIcon = (name) => {
    ...
    render() {
        const styles = {
            ...(this.color && { '--bq-icon--color': index.getColorCSSVariable(this.color) }),
            ...(this.size && { '--bq-icon--size': `${this.size}px` }),
        };
        return (index$1.h(index$1.Host, { key: '58b7f0bb84fc48079615e5e54e0e2f7f7fe62ed3', style: styles }, index$1.h("div", { key: 'ea1ebb6a5ea7d0be4c0101d48d3de2f4a816dbfd', "aria-label": this.label ?? `${this.name} icon`, class: `flex text-[color:--bq-icon--color] ${(this.size && 'bs-[--bq-icon--size] is-[--bq-icon--size]') || ''}`, innerHTML: this._svgContent, part: "base", role: "img" })));
    }
...
BqIcon.style = bqIconCss;

exports.bq_icon = BqIcon;

//# sourceMappingURL=bq-icon.cjs.entry.js.map